    ...
    for (l = 0; l <= n; l++) { // 计算长度为l的连续子序列，l最多是n
        ...
        for (low = id; low + l <= n; low += p) {
            ...
            if (low == high) { // l为0即只有1个结点
                ...
                // if (l == 4) {
                //     printf("low=%d, high=%d, bestcost=%lf\n", low, high,
                //            bestcost);
                //     fflush(stdout);
                // }
                cost[low][high] = bestcost;
                root[low][high] = bestroot;
                // printf("%d %d %d\n", low, high, root[low][high]);
                // fflush(stdout);
            }
            // 不能在这里就广播！！！
            // MPI_Bcast(&cost[low][high], 1, MPI_FLOAT, low % p,
            // MPI_COMM_WORLD); MPI_Bcast(&root[low][high], 1, MPI_INT, low % p,
            // MPI_COMM_WORLD);
        }
        for (low = 0; low + l <= n; low++) {
            high = low + l;
